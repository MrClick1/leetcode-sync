...
**146. LRU 缓存（2026-09-01）**

- 目录：problems/2026-09-01/0146-lru-cache/
- problem.md 只包含原题面；solution.cpp 已添加容量、哈希表和头节点，put、节点移动及淘汰逻辑尚未完成。
- test.cpp 覆盖官方示例、容量为 1、更新已有键刷新使用顺序、get 刷新使用顺序、查询缺失键和最大容量。
- 当前实现可以编译，但 get 命中分支缺少返回值，固定测试仍按预期在官方示例失败。
