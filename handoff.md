...
- 目录：problems/2026-09-02/0023-merge-k-sorted-lists/
- problem.md 只包含原题面；solution.cpp 使用由 LeetCode 和本地测试提供的 ListNode，避免重复定义。
- test.cpp 验证合并结果完整、有序、节点数量正确且无环，不强制必须复用原节点。
- 固定用例覆盖官方示例、空数组、空链表、重复值、边界值、10000 个链表和总计 10000 个节点。
- 当前已在 Solution 内定义 Compare 最小堆比较器，并处理 lists 为空的情况；非空输入仍返回 nullptr，尚未完成合并。
- 续做位置：在 mergeKLists 中创建最小堆，加入非空链表头，再用 dummy/tail 拼接弹出的节点并补入后继节点。
- 测试框架可以编译；当前 WIP 版本尚未通过官方示例，不能标记为已完成。
- C++ 语法文档已新增 priority_queue 的大顶堆、小顶堆、自定义比较器、常用接口和复杂度。
- 用户写完后执行 review → 固定测试 → 随机验证 → 更新复盘笔记和索引。
...
- 146 的代码、测试、复盘笔记和索引已经完成；同步状态以 git status 为准。
