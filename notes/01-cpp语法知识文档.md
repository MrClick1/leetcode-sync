...
n 为元素个数；R 为计数排序的取值范围大小；b 为桶数量；d 为基数排序的位数；r 为每位的基数。以下为空间和实现方式注明条件后的常见版本。

| 算法 | 最好时间 | 平均时间 | 最坏时间 | 额外空间 | 稳定性 |
|---|---|---|---|---|---|
| 冒泡排序（带提前退出） | O(n) | O(n²) | O(n²) | O(1) | 稳定 |
| 选择排序（交换最小值） | O(n²) | O(n²) | O(n²) | O(1) | 不稳定 |
| 插入排序 | O(n) | O(n²) | O(n²) | O(1) | 稳定 |
| 归并排序（普通数组版） | O(n log n) | O(n log n) | O(n log n) | O(n) | 稳定 |
| 快速排序（普通二路划分） | O(n log n) | O(n log n) | O(n²) | 平均 O(log n)，最坏 O(n) 递归栈 | 不稳定 |
| 堆排序 | O(n log n) | O(n log n) | O(n log n) | O(1) | 不稳定 |
| 计数排序（稳定输出版本） | O(n+R) | O(n+R) | O(n+R) | O(n+R) | 稳定 |
| 桶排序（桶内插入排序） | O(n+b) | 均匀分布时 O(n+b+n²/b) | O(n²+b) | O(n+b) | 取决于桶内排序和入桶方式 |
| 基数排序（逐位稳定计数排序） | O(d(n+r)) | O(d(n+r)) | O(d(n+r)) | O(n+r) | 稳定 |

桶数 b 与 n 同阶且分布均匀时，桶排序平均 O(n)；元素集中到一个桶时可能退化至 O(n²)。希尔排序的复杂度依赖增量序列，不能统一写一个平均复杂度；经典每次折半的增量序列最坏 O(n²)，额外空间 O(1)，不稳定。

稳定性指排序键相等的元素是否保持原来的相对顺序。表中“稳定”要求实现时不无故交换相等元素，例如归并时键相等优先取左侧。

#### C++ sort 与刷题选型

C++11 起，std::sort 要求最坏 O(n log n) 次比较，常见实现采用内省排序（结合快排、堆排和插排），不能直接把它当作最坏 O(n²) 的普通快速排序。std::sort 不保证稳定性；需要稳定排序可使用 std::stable_sort。

复杂度通常假定比较元素为 O(1)。排序 vector<string> 或 vector<vector<int>> 时，一次比较可能扫描多个字符或元素，因此 O(n log n) 指比较次数，完整时间还应计入比较成本。

计数、桶、基数排序依赖值域、分布或位数等条件，并非任意输入都能无条件线性排序。一般比较排序的最坏比较次数下界为 Ω(n log n)。
